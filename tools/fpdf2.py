...
class PDF(FPDF):
    def header(self):
        ...
        if Tamano == False:
            self.image(os.path.join(imagenes, "sintitulo.png"), 20, 5, 250, 35)
        elif Tamano == True:
...
def InventarioQuery(listado, titulo):
    global Titulo, Tamano
    Titulo = titulo
    Tamano = True #Vertical True
...
def entradasQuery(listado, titulo):
    global Titulo
    Titulo = titulo
    global Tamano
    Tamano = False #Vertical True
    # Instantiation of inherited class
    pdf = PDF("L", 'mm', 'LETTER')
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    epw = pdf.w - 2 * pdf.l_margin
    col_width = epw / 12
    data3 = ('Id', 'Proveedor', 'Fol. Entrada', 'Fecha', 'Factura', 'Núm. Factura', 'Orden Comp', 'Dep. Soli', 'Núm. Req.', 'Ofi. Soli', 'Total', 'Observaciones')
    ...
    for item in data3:
        if item == "Id":
            pdf.cell(col_width-15, th, str(item), fill=True,border=1,align='C')
        elif item == "Proveedor":
            pdf.cell(col_width+25, th, str(item), fill=True,border=1,align='C')
        elif item == 'Fol. Entrada':
            pdf.cell(col_width-5, th, str(item), fill=True,border=1,align='C')
        elif item == 'Fecha':
            pdf.cell(col_width-10, th, str(item), fill=True,border=1,align='C')
        elif item == 'Factura':
            pdf.cell(col_width-10, th, str(item), fill=True,border=1,align='C')
        elif item == 'Orden Comp':
            pdf.cell(col_width-5, th, str(item), fill=True,border=1,align='C')
        elif item == 'Núm. Req.':
            pdf.cell(col_width-10, th, str(item), fill=True,border=1,align='C')
        elif item == 'Ofi. Soli':
            pdf.cell(col_width+15, th, str(item), fill=True,border=1,align='C')
        elif item == 'Observaciones':
            pdf.cell(col_width+15, th, str(item), fill=True,border=1,align='C')
        else:
            pdf.cell(col_width, th, str(item), fill=True,border=1,align='C')
    pdf.ln()
    #########################################
    ###       Cuerpo del procedimiento    ###
    #########################################
    ...
    for item in listado:
        if i%2==0:
            pdf.set_fill_color(184, 188, 191)
            pdf.cell(col_width-15, th+2, str(item.id), fill=True,border=1,align='C')
            pdf.cell(col_width+25, th+2, str(item.proveedor),fill=True,border=1,align='C')
            pdf.cell(col_width-5, th+2, str(item.fol_entrada),fill=True,border=1,align='C')
            pdf.cell(col_width-10, th+2, str(item.fecha),fill=True,border=1,align='C')
            pdf.cell(col_width-10, th+2, "Factura" if item.factura == "F" else "Nota" ,fill=True,border=1,align='C')
            pdf.cell(col_width, th+2, (item.nFactura),fill=True,border=1,align='C')
            pdf.cell(col_width-5, th+2, (item.ordenCompra),fill=True,border=1,align='C')
            pdf.cell(col_width, th+2, (item.depSolici[:20]),fill=True,border=1,align='C')
            pdf.cell(col_width-10, th+2, (item.nReq),fill=True,border=1,align='C')
            pdf.cell(col_width+15, th+2, (item.oSolicitnte ),fill=True,border=1,align='C')
            pdf.cell(col_width, th+2, SetMoneda(item.total,'$',2),fill=True,border=1,align='C')
            pdf.cell(col_width+15, th+2, (item.observaciones[:30]),fill=True,border=1,align='C')
            pdf.ln()
        else:
            pdf.set_fill_color(255, 255, 255)
            pdf.cell(col_width-15, th+2, str(item.id), fill=True,border=1,align='C')
...
def salidasQuery(listado, titulo):
    global Titulo
    Titulo = titulo
    global Tamano
    Tamano = False #Vertical True
